    ...
    return (
        <>
            <ContainerMain className="navbar navbar-expand-md navbar-dark">
                ...
                <p className="ml-2" hidden={props.tab !== 'Início' ? true : false}><strong>#{useSelector(state => state.login)}</strong></p>
                ...
                <p style={{whiteSpace: 'nowrap', marginRight: '1rem', fontSize: '1.3rem'}} hidden={props.tab === 'Início' ? true : false}><strong>{useSelector(state => {
                    switch(props.tab){
                        case 'Repositórios':
                ...
                <Link className="d-flex align-items-center" style={{ padding: '1rem 2rem 1rem 1rem'}} to="/" onClick={()=>{localStorage.removeItem('persist:user');}}>
                    Sair
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-box-arrow-right text-danger mx-2" viewBox="0 0 16 16">
